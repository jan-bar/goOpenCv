...
func main() {
	...
	src := opencv.LoadImage(*s, opencv.CV_LOAD_IMAGE_GRAYSCALE)
	temp := opencv.LoadImage(*t, opencv.CV_LOAD_IMAGE_GRAYSCALE)
	...
	opencv.Threshold(ftmp, ftmp, 0.99, 1.0, opencv.CV_THRESH_TOZERO)
	...
	for {
		...
		if maxVal >= 0.8 {
			...
			opencv.Rectangle(src, pt1, pt2, opencv.NewScalar(0, 0, 0, 0))
